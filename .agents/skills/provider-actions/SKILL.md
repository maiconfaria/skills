...
description: Use when implementing Terraform Provider actions using the Plugin Framework. Use when developing imperative operations that execute at lifecycle events (before/after create, update, destroy).
metadata:
  author: IBM
  source: https://github.com/hashicorp/agent-skills
  source-path: terraform/provider-development/skills/provider-actions
