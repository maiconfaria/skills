...
description: Use when creating, modifying, or validating HashiCorp Terraform Stack configurations (.tfcomponent.hcl, .tfdeploy.hcl files). Use when working with stack components and deployments from local modules, public registry, or private registry sources. Use when managing multi-region or multi-environment infrastructure or troubleshooting Terraform Stacks syntax and structure.
metadata:
  author: IBM
  source: https://github.com/hashicorp/agent-skills
  source-path: terraform/module-generation/skills/terraform-stacks
